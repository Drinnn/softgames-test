...
jest.mock('./dtos', () => ({
  validateGetGameByIdDto: jest.fn()
}));

describe('GetGameByIdController', () => {
  let mockExecute: jest.Mock;
  let mockService: GetGameByIdService;
  let controller: GetGameByIdController;
  ...
  beforeEach(() => {
    ...
    mockService = {
      execute: mockExecute
    } as unknown as GetGameByIdService;
    
    controller = new GetGameByIdController(mockService);
  ...
  describe('handle', () => {
    it('should return 200 and game data when validation and execution succeed', async () => {
      ...
      (dtos.validateGetGameByIdDto as jest.Mock).mockReturnValue({
        success: true,
        data: {
          gameId: validGameId
        }
      ...
      expect(dtos.validateGetGameByIdDto).toHaveBeenCalledWith({
        gameId: validGameId
      });
